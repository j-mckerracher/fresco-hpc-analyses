#!/bin/bash
# Processing script for 2014 FRESCO dataset
# Modified to process only 2014 with enhanced logging
...
MAX_WORKERS=3
DATA_ROOT="../data"
LOG_FILE="processing_2014_$(date +%Y%m%d_%H%M%S).log"

echo "Starting processing of 2014 FRESCO dataset"
...
process_year() {
    ...
    if [ $? -eq 0 ]; then
        ...
        log_msg "✗ Year $year failed at $(date)"
        return 1
    fi
    log_msg ""
}
